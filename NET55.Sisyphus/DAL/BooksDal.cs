...
namespace Maticsoft.DAL
{
	...
	public partial class BooksDal
	{
		...
		{
            string sql = "select * from(select *,ROW_NUMBER() over(order by Clicks desc)as num from  [books]) as t where t.num>@stact and t.num<=@end";
			StringBuilder strSql=new StringBuilder();
			...
			{
				strSql.Append("order by T.PublishDate desc");
			}
        ...
        {
            string sql = "select top "+count+" * from books order by NEWID()";
        ...
        {
            string sql = "select * from books where title like '%@word%'";
            SqlParameter[] sp = { 
                                    new SqlParameter("@word",searchWord)
                                };
        ...
        {
            string sql = "select top 1 * from Books order by PublishDate desc";
            Maticsoft.Model.Books model = new Maticsoft.Model.Books();
            DataSet ds = DbHelperSQL.Query(sql);
            if (ds.Tables[0].Rows.Count > 0)
            {
        ...
        public DataSet TodayTuiJian() {
            string sql = "select top 3 * from Books order by Clicks desc";
            DataSet ds = DbHelperSQL.Query(sql);
            return ds;
        }
